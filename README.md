...
### Registration
- `POST /registration/register` – Register for an event (Student only)
- `GET /registration/checkin/{qr_id}` – Mark user as checked-in (Scan endpoint)

### Admin
- `GET /admin/attendees/{event_id}` – View all registered users for an event
- `GET /admin/stats/{event_id}` – View check-in stats
